 ...
 */
@WebServlet(urlPatterns = {"/listado_edad"})
public class listado_edad extends HttpServlet {
Controlador controlador = new Controlador();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            List <Persona> resp = controlador.Listado(Integer.valueOf(request.getParameter("listaEdad")));
            out.println("<!DOCTYPE html>");
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet listado_edad</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Listado filtrado por edad:</h1>");
